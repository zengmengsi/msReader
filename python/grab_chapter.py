...
class QWERTYU:
    ...
    def start(self):
        ...
        arr = []
        pageType = sys.argv[1];
        url = 'http://m.quanshuwang.com/list/'+sys.argv[1]+'_'+sys.argv[2]+'.html'
        # url= 'http://www.quanshuwang.com/book/' + str(pageType)
        ...
        # pattern = re.compile('<li><a href="(.\d*?).html" title=".*?">(.*?)</a></li>',re.S)
        pattern = re.compile('<li><a href="/mbook/(.*?).html"  title=".*?">(.*?)</a>',re.S)
