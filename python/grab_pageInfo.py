...
class QWERTYU:
    ...
    def start(self):
        ...
        pattern = re.compile('<img class="book_cover" src="(.*?)" on.*?">.*?<h1>(.*?)</h1>.*?<ul class="intro"><li>.*?<a href=".*?">(.*?)</a></li>.*?<li id="hits">(.*?)</li>.*?<li>.*?<span>(.*?)</span>.*?<li>.*?<a  class="begin" href="/list/(.*?)_.*?.html">.*?<div class="detail_p">.*?<div>(.*?)</div>',re.S)
