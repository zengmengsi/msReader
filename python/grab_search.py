...
class QWERTYU:
    ...
    def start(self):
        ...
        arr = []
        search = sys.argv[1];
        print search.encode('gbk')
        url= 'http://m.quanshuwang.com/modules/article/search.php?searchkey='+search+'type='
        ...
        pattern = re.compile('<li>.*?<a href="/book_(.*?).html">.*?<img.*?src="(.*?)"><p class="book_title">(.*?)</p>.*?<p><span class="status">(.*?)</span>(.*?)</p>.*?<p>(.*?)</p>.*?<p>.*?</p>.*?</a>.*?</li>',re.S)
