<template>
    <div id="container" class="container">
        ...
        <v-touch class="content" @tap="operationAction($event)"
                 :class="[mySetting.bgcolorClass,{'night-mode':mySetting.nightMode}]">
            <h3>{{bookChaptersContent.links}}</h3>
            <article v-html="bookChaptersBody"></article>
        </v-touch>
        <div class="menu" v-if="operation">
            <v-touch class="menu-btn" v-if="mySetting.nightMode" @tap="changeMode">
        ...
        <div class="read-setting" v-show="isShowSet">
            <div class="setting-item">
                <v-touch tag="span" @tap="mySetting.curfontsize -= 0.1;setFont()">
                    <i class="iconfont">&#xe605;</i>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.curfontsize = 1;setFont()">
                    <i class="iconfont">&#xe8eb;</i>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.curfontsize += 0.1;setFont()">
                    <i class="iconfont">&#xe603;</i>
            ...
            <div class="setting-item" style="height: 3rem">
                <v-touch tag="span" @tap="mySetting.bgcolorClass='huyan';setLocal()">
                    <div class="bg-color" style="background-color: #faf6ed"></div>
                    <div class="bg-text">护眼</div>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.bgcolorClass='qinxin';setClass()">
                    <div class="bg-color" style="background-color: #ceebcf"></div>
                    <div class="bg-text">清新</div>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.bgcolorClass='haijun';setClass()">
                    <div class="bg-color" style="background-color: #09598c"></div>
                    <div class="bg-text">海军蓝</div>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.bgcolorClass='zhihuang';setClass()">
            ...
            <div class="line1pix"></div>
            <div class="setting-item">
                <v-touch tag="span" @tap="mySetting.curlineheight+=0.2;setLineHeight()">
                    <i class="iconfont">&#xe608;</i>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.curlineheight=1.5;setLineHeight()">
                    <i class="iconfont">&#xe607;</i>
                </v-touch>
                <v-touch tag="span" @tap="mySetting.curlineheight-=0.2;setLineHeight()">
                    <i class="iconfont">&#xe602;</i>
        ...
        <div class="chapter-list" v-show="isShowChapter" v-scroll="onScroll">
            ...
            <ul id="chapter-list" v-if="loadedChapters">
                <v-touch tag="li" v-for="(item, index) in loadedChapters" :key="index" @tap="jumpChapter(index)">
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.mySetting = util.getLocalStroageData('mySetting')?util.getLocalStroageData('mySetting'):this.mySetting;
            this.setFont();
            this.setLineHeight();
        },
        // mounted () {
        ...
        methods: {
            ...
            // 切换日间/夜间模式
            changeMode() {
                this.mySetting.nightMode = !this.mySetting.nightMode
                util.setLocalStroageData('mySetting', this.mySetting)
            },
            // 显示目录
            showChapter() {
...
//      util.setLocalStroageData('followBookList', readRecord)
            ...
            setFont() {
                util.setLocalStroageData('mySetting', this.mySetting)
                document.getElementsByTagName('article')[0].style.fontSize = this.mySetting.curfontsize + 'rem'
            },
            /**
             * 设置行间距
             */
            setLineHeight() {
                util.setLocalStroageData('mySetting', this.mySetting)
                document.getElementsByTagName('article')[0].style.lineHeight = this.mySetting.curlineheight + 'rem'
            },
            /**
             * 设置皮肤
             */
